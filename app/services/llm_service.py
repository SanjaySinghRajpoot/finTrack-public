...
Your task:
- First, determine if the provided text is relevant to the context of the JSON schema (e.g., financial, invoice, payment, or billing-related information). 
  - If the text is unrelated or does not contain enough information to extract the required fields, set `is_processing_valid` to false and leave other fields empty.
- If the content is relevant, extract the required and optional information from the text.
- Populate the JSON schema with the extracted values.
- Ensure that the output strictly follows the JSON structure.
- Do not add any extra keys.
- Keep values empty if they cannot be found in the text.
...
# Schema definition
schema = {
    "is_processing_valid": {"type": "boolean", "description": "Whether the document is valid and relevant to the context", "default": False},
    "document_type": {
        "type": "string",
...
class LLMService:
    ...
    def validate_json(self, data: dict, schema: dict, required: list) -> dict:
        """
    ...
    def call_gemini(self, text_content: str) -> dict:
        """
        Calls Gemini API with the given text content asynchronously,
        ...
        try:
            ...
            validated_json = self.validate_json(response_json, schema, required_fields)
